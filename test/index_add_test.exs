defmodule Discord.SortedSet.IndexAdd.Test do
  use ExUnit.Case
  ...
  describe "set behavior" do
    test "single item" do
      ...
      assert {0, ^set} = SortedSet.index_add(set, 5)
    ...
    test "100 unique items" do
      ...
      for i <- 1..100 do
        index = i - 1
        assert {^index, ^set} = SortedSet.index_add(set, i)
    ...
    test "same item 100 times" do
      ...
      for _ <- 1..100 do
        assert {nil, ^set} = SortedSet.index_add(set, 5)
        assert SortedSet.size(set) == 1
      end
    end
    ...
    test "mix of unique and duplicate items" do
      ...
      assert {100, ^set} = SortedSet.index_add(set, 5555)

      for _ <- 1..100 do
        assert {nil, ^set} = SortedSet.index_add(set, 5555)
  ...
  describe "supported data types" do
    test "integer" do
      assert set = SortedSet.new()
      assert {0, ^set} = SortedSet.index_add(set, 15)
    ...
    test "tuple of integer" do
      assert set = SortedSet.new()
      assert {0, ^set} = SortedSet.index_add(set, {1, 2, 3})
    ...
    test "list of tuple" do
      assert set = SortedSet.new()
      assert {0, ^set} = SortedSet.index_add(set, [{:a, :b}, {1, 2}, {"a", "b"}])
    ...
    @tag timeout: 240_000
    test "any supported term can be successfully added to the set" do
  ...
  describe "unsupported data types" do
    test "float" do
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, 1.2)
    end

    test "reference" do
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, make_ref())
    end

    test "pid" do
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, self())
    end

    test "port" do
      assert port = Port.open({:spawn, "cat"}, [:binary])
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, port)
      assert true = Port.close(port)
    end

    test "function" do
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, & &1)
    end

    test "tuple of float" do
      value = 1.2
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, {value, value})
    end
    ...
    test "tuple of mixed unsupported" do
      ...
      assert {:error, :unsupported_type} =
               SortedSet.index_add(set, {1.2, make_ref(), self(), port, & &1})
    ...
    test "tuple of mixed supported and unsupported terms" do
      assert set = SortedSet.new()
      assert {:error, :unsupported_type} = SortedSet.index_add(set, {1, :a, 3.4, make_ref()})
    end
    ...
    test "tuple of list of unsupported" do
      ...
      assert {:error, :unsupported_type} =
               SortedSet.index_add(set, {[1.2, make_ref()], [self(), & &1]})
    end
    ...
    @tag timeout: 240_000
    test "any unsupported term added to the set results in an error" do
